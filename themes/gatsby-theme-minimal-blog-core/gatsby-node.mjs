...
import { kebabCase, mdxResolverPassthrough, slugify } from "@lekoarts/themes-utils"
...
export function createSchemaCustomization({ actions }, themeOptions) {
...
export function sourceNodes({ actions, createContentDigest }, themeOptions) {
...
export function onCreateNode({ node, actions, getNode, createNodeId, createContentDigest }, themeOptions) {
	...
	if (node.internal.type === `Mdx` && source === pagesPath) {
		const fieldData = {
			title: node.frontmatter.title,
			slug: `/${basePath}/${node.frontmatter.slug}`.replace(/\/{2,}/g, `/`),
			defer: node.frontmatter.defer,
			contentFilePath: fileNode.absolutePath,
			timeToRead: roundReadingTime(readingTimeResult.minutes),
			wordCount: readingTimeResult.words,
...
export async function createPages({ actions, graphql, reporter }, themeOptions) {
	...
	createPage({
		path: `/${basePath}/${blogPath}`.replace(/\/{2,}/g, `/`),
	...
	createPage({
		path: `/${basePath}/${tagsPath}`.replace(/\/{2,}/g, `/`),
  ...
  `)
	...
	if (tags.length > 0) {
		tags.forEach((tag) => {
			createPage({
				path: `/${basePath}/${tagsPath}/${kebabCase(tag.fieldValue)}`.replace(/\/{2,}/g, `/`),
