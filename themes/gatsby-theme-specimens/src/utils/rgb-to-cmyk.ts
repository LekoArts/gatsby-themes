 ...
 */
function RGBToCMYK(R: number, G: number, B: number): { C: number, M: number, Y: number, K: number } | undefined {
	let c
	let m
	let y
	let k

	const r = Number.parseInt(`${R}`.replace(/\s/g, ``), 10)
	const g = Number.parseInt(`${G}`.replace(/\s/g, ``), 10)
	const b = Number.parseInt(`${B}`.replace(/\s/g, ``), 10)
